...
#Pneu_coding = pd.DataFrame(columns=['Desc','Code'])
#Pneu_coding['Desc'] = mkt_data_RT_melt2['Pneu']
#Pneu_coding['Code'] = le.fit_transform(mkt_data_RT_melt2['Pneu'].astype(str))
#Country_coding = mkt_data_RT_melt2['Country_Code']
mkt_data_RT_melt2['Pneu'] = le.fit_transform(mkt_data_RT_melt2['Pneu'].astype(str))
...
#mkt_data_RT_melt2['QTY_RT'].hist(bins=20, figsize=(10,5))
...
for Year in range(2016,2019):
    ...
    xts['Prev'] = p
    xts['Pneu'] = le.inverse_transform(xts['Pneu'])
...
mkt_data_RT_melt2['Pneu'] = le.inverse_transform(mkt_data_RT_melt2['Pneu'])

Eval_Year = Eval[Eval.Year == 2016]
...
mkt_data_RT_melt2 = mkt_data_RT_melt[mkt_data_RT_melt.Country_Code == country]
mkt_data_RT_melt2 = mkt_data_RT_melt2.drop(['Country_Code'], axis=1)  
mkt_data_RT_melt2.rename(columns={'Qty_month':'QTY'}, inplace=True)
mkt_data_RT_melt2[mkt_data_RT_melt2['QTY'] < 0] = 0 #Remove negative values - what does it means?
mkt_data_RT_melt2['QTY_1'] = mkt_data_RT_melt2.groupby(['Pneu'])['QTY'].shift()
mkt_data_RT_melt2['QTY_1_diff'] = mkt_data_RT_melt2.groupby(['Pneu'])['QTY_1'].diff()
mkt_data_RT_melt2.fillna(0, inplace=True)
    #mkt_data_RT_melt2 = mkt_data_RT_melt2.dropna()
le = preprocessing.LabelEncoder()
mkt_data_RT_melt2['Pneu'] = le.fit_transform(mkt_data_RT_melt2['Pneu'].astype(str))
...
for year in range(2016,2019):
    train = mkt_data_RT_melt2[mkt_data_RT_melt2['Year'] < year]
    val = mkt_data_RT_melt2[mkt_data_RT_melt2['Year'] == year]
    error = rmsle(val['QTY'].values, val['QTY_1'].values)
    print('Year %d - Error %.5f' % (year, error))
    mean_error.append(error)
print('Mean Error = %.5f' % np.mean(mean_error))

mkt_data_RT_melt2['QTY'].hist(bins=20, figsize=(10,5))
